...
export default {
    ...
    activated(){//当keep-alive组件激活时调用,及切换回来时会被再次调用，而mounted只会调用一次
        var cityId = this.$store.state.city.id;

        if (this.prevCityId === cityId) {
            return;//如果没有切换城市，则ID不变，就不会执行下面的请求代码，否则执行代码切换城市数据
        }
         this.isLoading = true;
        ...
        this.axios.get('/api/movieOnInfoList?cityId='+cityId).then((res)=>{
            ...
            if (msg === 'ok') {
                ...
                /* this.$nextTick(()=>{//解决滑动流畅度
                    var scroll = new BScroll ( this.$refs.movie_body, {
                        tap : true,//tag事件配置参数
                        probeType : 1//触发scroll事件配置参数
                    });
